...
import org.zcdada.shortlink_zc.project.common.user.UserContext;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    private final ShortLinkGroupMapper linkGroupMapper;
    private final ShortLinkAccessStatsMapper linkAccessStatsMapper;
    private final ShortLinkLocaleStatsMapper linkLocaleStatsMapper;
    private final ShortLinkAccessLogsMapper linkAccessLogsMapper;
    private final ShortLinkBrowserStatsMapper linkBrowserStatsMapper;
    private final ShortLinkOsStatsMapper linkOsStatsMapper;
    private final ShortLinkDeviceStatsMapper linkDeviceStatsMapper;
    private final ShortLinkNetworkStatsMapper linkNetworkStatsMapper;
    ...
    public ShortLinkStatsRespDTO oneShortLinkStats(ShortLinkStatsReqDTO requestParam) {
        ...
        // 基础访问数据
        List<ShortLinkAccessStatsDO> listStatsByShortLink = linkAccessStatsMapper.listStatsByShortLink(requestParam);
        if (CollUtil.isEmpty(listStatsByShortLink)) {
            return null;
        }
        // 基础访问详情 pv，uv，uip
        ShortLinkAccessStatsDO pvUvUidStatsByShortLink = linkAccessLogsMapper.findPvUvUidStatsByShortLink(requestParam);
        ...
        // 地区访问详情（仅国内） province 访问省份的详情
        List<ShortLinkStatsLocaleCNRespDTO> localeCnStats = new ArrayList<>();
        List<ShortLinkLocaleStatsDO> listedLocaleByShortLink = linkLocaleStatsMapper.listLocaleByShortLink(requestParam);
//        访问数总和
...
//        计算比例
        ...
        List<ShortLinkAccessStatsDO> listHourStatsByShortLink = linkAccessStatsMapper.listHourStatsByShortLink(requestParam);
//        这个循环好像写的有点蠢啊？ 检查listHourStatsByShortLink查出来的是什么情况
        for (int i = 0; i < 24; i++) {
        ...
        List<ShortLinkDeviceStatsDO> listDeviceStatsByShortLink = linkDeviceStatsMapper.listDeviceStatsByShortLink(requestParam);
        ...
        List<ShortLinkNetworkStatsDO> listNetworkStatsByShortLink = linkNetworkStatsMapper.listNetworkStatsByShortLink(requestParam);
    ...
    public ShortLinkStatsRespDTO groupShortLinkStats(ShortLinkGroupStatsReqDTO requestParam) {
        ...
        List<ShortLinkAccessStatsDO> listStatsByGroup = linkAccessStatsMapper.listStatsByGroup(requestParam);
        if (CollUtil.isEmpty(listStatsByGroup)) {
            return null;
        }
        // 基础访问数据
        ShortLinkAccessStatsDO pvUvUidStatsByGroup = linkAccessLogsMapper.findPvUvUidStatsByGroup(requestParam);
        // 基础访问详情
        List<ShortLinkStatsAccessDailyRespDTO> daily = new ArrayList<>();
        List<String> rangeDates = DateUtil.rangeToList(DateUtil.parse(requestParam.getStartDate()), DateUtil.parse(requestParam.getEndDate()), DateField.DAY_OF_MONTH).stream()
                .map(DateUtil::formatDate)
        ...
        // 地区访问详情（仅国内）
        List<ShortLinkStatsLocaleCNRespDTO> localeCnStats = new ArrayList<>();
        List<ShortLinkLocaleStatsDO> listedLocaleByGroup = linkLocaleStatsMapper.listLocaleByGroup(requestParam);
        ...
        // 小时访问详情
        List<Integer> hourStats = new ArrayList<>();
        List<ShortLinkAccessStatsDO> listHourStatsByGroup = linkAccessStatsMapper.listHourStatsByGroup(requestParam);
        for (int i = 0; i < 24; i++) {
            AtomicInteger hour = new AtomicInteger(i);
        ...
        // 一周访问详情
        List<Integer> weekdayStats = new ArrayList<>();
        List<ShortLinkAccessStatsDO> listWeekdayStatsByGroup = linkAccessStatsMapper.listWeekdayStatsByGroup(requestParam);
        for (int i = 1; i < 8; i++) {
            AtomicInteger weekday = new AtomicInteger(i);
        ...
        // 访问设备类型详情
        List<ShortLinkStatsDeviceRespDTO> deviceStats = new ArrayList<>();
        List<ShortLinkDeviceStatsDO> listDeviceStatsByGroup = linkDeviceStatsMapper.listDeviceStatsByGroup(requestParam);
        ...
        // 访问网络类型详情
        List<ShortLinkStatsNetworkRespDTO> networkStats = new ArrayList<>();
        List<ShortLinkNetworkStatsDO> listNetworkStatsByGroup = linkNetworkStatsMapper.listNetworkStatsByGroup(requestParam);
